...
def draw_beam():
    ...
    btn.draw()
    btn.get_tk_widget().grid(row=3,column=4,rowspan=7,columnspan=6)
...
def testing():
    if typ.get():
        ...
        btn2.get_tk_widget().grid(row=0,column=0,columnspan=8,rowspan=8)
        plt.figure()
...
lbl_fr=TK.LabelFrame(content,text="ابعاد زیردریایی ",background=about_panel._bg_color[1],fg=about_panel._fg_color[0])
...
set_but.grid(column=0,row=0,padx=padx,pady=pady)

typ=tk.IntVar()
typ.set(0)
rad_lin=tk.CTkRadioButton(set_panel,text="غیرفعال ",value=0,variable=typ,font=font)
rad_plan=tk.CTkRadioButton(set_panel,text="فعال ",value=1,variable=typ,font=font)
rad_lin.grid(column=2,row=0)
rad_plan.grid(column=1,row=0)

vel=tk.CTkEntry(set_panel,textvariable=velocity)
vel.grid(column=3,row=0)
lbl =tk.CTkLabel(set_panel,text="(m/s)سرعت صوت   ",font=font)
lbl.grid(column=4,row=0)

marine_dia= tk.CTkEntry(set_panel,textvariable=d)
marine_dia.grid(column=5,row=0)
lbl =tk.CTkLabel(set_panel,text="(m)قطر  ",font=font)
lbl.grid(column=6,row=0)

marine_heigth= tk.CTkEntry(set_panel,textvariable=h)
marine_heigth.grid(column=7,row=0)
lbl =tk.CTkLabel(set_panel,text="(m)ارتفاع  ",font=font)
lbl.grid(column=8,row=0)

marine_length= tk.CTkEntry(set_panel,textvariable=l)
marine_length.grid(column=9,row=0)
lbl =tk.CTkLabel(set_panel,text="(m)طول  ",font=font)
lbl.grid(column=10,row=0)

# option_panel(content)
lbl_fr_opt=TK.LabelFrame(content,text="مشخصات سونار جانبی",background=about_panel._bg_color[1],fg=about_panel._fg_color[0])
...
cal_btn.grid(column=4,row=0,padx=padx,pady=pady)
d_sen=tk.CTkEntry(opt_panel,textvariable=dis)
d_sen.grid(column=5,row=0)
lbl=tk.CTkLabel(opt_panel,text="فاصله سنسورها  ",font=font)
lbl.grid(column=6,row=0)

F_s=tk.CTkEntry(opt_panel,textvariable=fs)
F_s.grid(column=7,row=0)
lbl=tk.CTkLabel(opt_panel,text="نرخ نمونه برداری  ",font=font)
lbl.grid(column=8,row=0)

num_sen=tk.CTkEntry(opt_panel,textvariable=M)
num_sen.grid(column=9,row=0)
lbl=tk.CTkLabel(opt_panel,text="تعداد سنسورها  ",font=font)
lbl.grid(column=10,row=0)

lbl=tk.CTkLabel(opt_panel,text="2-100",state='disabled')
lbl.grid(column=9,row=1,sticky='nw')
lbl=tk.CTkLabel(opt_panel,text="2-50 KHz",state='disabled')
lbl.grid(column=7,row=1,sticky='nw')
...
beam_btn.grid(row=2,column=4,padx=padx,pady=pady)
...
lbl_fr_out=TK.LabelFrame(content,text="تحلیل عملکرد",background=about_panel._bg_color[1],fg=about_panel._fg_color[0])
lbl_fr_out.grid(column=2,row=3,columnspan=2,sticky="NSEW")
...
amb_noise=tk.CTkCheckBox(out_panel,text="Ambition Noise نویز محیط",font=font)
amb_noise.grid(row=0,column=10,sticky="w")
clutter=tk.CTkCheckBox(out_panel,text="Clutter شبهه هدف",font=font)
clutter.grid(row=1,column=10,sticky="w")
reverb=tk.CTkCheckBox(out_panel,text="Reverbtion طنین",font=font)
reverb.grid(row=2,column=10,sticky="w")
lbl=tk.CTkLabel(out_panel,text="سطح نویز محیط  ",font=font)
lbl.grid(row=0,column=9)
lbl=tk.CTkLabel(out_panel,text="احتمال شبهه هدف  ",font=font)
lbl.grid(row=1,column=9)
lbl=tk.CTkLabel(out_panel,text="احتمال طنین  ",font=font)
lbl.grid(row=2,column=9)
A_snr=tk.CTkEntry(out_panel,textvariable=Nl)
A_snr.grid(row=0,column=8)
p_clut=tk.CTkEntry(out_panel,textvariable=Pc)
p_clut.grid(row=1,column=8)
p_reverb=tk.CTkEntry(out_panel,textvariable=Pr)
p_reverb.grid(row=2,column=8)
lbl=tk.CTkLabel(out_panel,text="تعداد نمونه‌ها",font=font)
lbl.grid(row=3,column=10)
n_sample=tk.CTkEntry(out_panel,textvariable=Ns)
n_sample.grid(row=3,column=9)
test=tk.CTkButton(out_panel,text="Test",command=testing)
test.grid(row=3,column=8,padx=padx,pady=pady)

# vis panel
lbl_fr_vis=TK.LabelFrame(content,text="پیش نمایش",background=about_panel._bg_color[1],fg=about_panel._fg_color[0])
lbl_fr_vis.grid(column=0,row=3,columnspan=2,sticky="NSEW")
